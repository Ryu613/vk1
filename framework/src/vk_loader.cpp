...
std::optional<std::vector<std::shared_ptr<MeshAsset>>> loadGltfMeshes(Context* ctx,
  ...
  if (load) {
    ...
    fmt::println("failedd to load glTF: {}", fastgltf::to_underlying(load.error()));
    return {};
  }
  ...
  for (fastgltf::Mesh& mesh : gltf.meshes) {
    ...
    for (auto&& p : mesh.primitives) {
      ...
      if (normals != p.attributes.end()) {
        fastgltf::iterateAccessorWithIndex<glm::vec3>(
            gltf, gltf.accessors[normals->accessorIndex], [&](glm::vec3 v, size_t index) {
      ...
      if (colors != p.attributes.end()) {
        fastgltf::iterateAccessorWithIndex<glm::vec4>(
            gltf, gltf.accessors[(*colors).accessorIndex], [&](glm::vec4 v, size_t index) {
    ...
    newmesh.meshBuffers = ctx->uploadMesh(indices, vertices);
